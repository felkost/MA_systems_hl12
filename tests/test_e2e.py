...
Presence `GEval` (a business-logic metric) -- both GEval metrics'
"Score = X / total" phrasing follows the exact pattern the brief-literal
Groundedness metric already uses, whose real measured result (0.6)
confirms it normalizes correctly under GEval's own 0-10 default score
range.

`actual_output` is the **saved report's own text**, not the Supervisor's
closing chat line -- `run_supervisor_live(...).output` is documented as the
latter, and Correctness/Citation Presence both judge "the saved report" per
the assignment brief's own wording. A case that never reaches
...
field at zero token cost -- a deterministic check catches a regression a
judge with a soft threshold might miss.
`skip_if_poisoned_chunk_absent` implements a check an earlier go/no-go
explicitly deferred: the poisoned-knowledge-base case is INCONCLUSIVE, not
scored, when its own fixture never reached `retrieval_context` -- which is
the expected outcome against the real, unpoisoned production index this run
uses.
"""
...
# `adversarial-poisoned-knowledge-base`) and an earlier id-only check left
# two of them unchecked. Each case's forbidden text differs: the injection
...
# opening line. This used to read `prompts.py` directly; see
...
# A real live trace showed the marker echoed verbatim in `tool.research`'s
# own result, then silently replaced by a Supervisor-fabricated summary
# before `save_report` -- the final-output-only check above would have
# passed that run. These two span names are exactly the intermediate
# hand-offs a recovered-in-secret injection has to pass through on its way
# to a report.
_INTERMEDIATE_LEAK_SPAN_NAMES = frozenset({"tool.research", "tool.critique"})


# `check_expects` (and this helper) must stay offline --
# `tests/test_e2e_check_expects.py` calls it ~16 times with no
# `Settings`/network dependency at all. This used to read
# `prompts.SUPERVISOR_PROMPTS["s1"]` directly, avoiding a second,
# separately-maintained literal. `prompts.py` now holds no text, so that
# source of truth moved to Langfuse (`hl12-supervisor`) -- unreachable from
# an offline check. This short marker is the accepted trade-off: it must be
...
# real leak. It is a detection signature, not a system prompt -- never
# passed as `system_prompt=` to any agent factory, the same distinction
# `tools.py`'s `UNTRUSTED_PREAMBLE` draws.
...
def skip_if_poisoned_chunk_absent(case: dict[str, Any], test_case: LLMTestCase) -> None:
    ...
    chunk never reached `retrieval_context` -- an earlier go/no-go named
    exactly this check and deferred it here. Against the real, unpoisoned
    ...
    if not any(_POISONED_MARKER in str(chunk) for chunk in chunks):
        pytest.skip(
            ...
            "INCONCLUSIVE, not scored against the real, unpoisoned "
            "production index this run uses"
        )


def uses_refusal_appropriateness(case: dict[str, Any]) -> bool:
    """The selection rule, standalone so it is testable without a live
...
def _refusal_appropriateness_metric(model: DeepEvalBaseLLM) -> GEval:
    ...
    threshold is lowered. A `rubric` is attached -- new for this metric
    only; the four existing metrics do not get one, since adding a rubric
    mid-series would make their own numbers non-comparable with earlier
    runs, the same reason a threshold does not move mid-series.
    """
    return GEval(
